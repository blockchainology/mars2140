...
class Game:
    ...
    def intro(self):
        ...
        self.clear_screen()
        self.type_text("╔══════════════════════════════════════════════════════════╗")
        self.type_text("║                BITCOIN MARS MYSTERIES 2140              ║")
        self.type_text("║              The Last Satoshi Conspiracy                 ║")
        self.type_text("╚══════════════════════════════════════════════════════════╝")
        print()
        
        self.type_text("Mars Colony New Nakamoto, Year 2140...")
        self.type_text("All 21 million Bitcoin have been minted for over a century.")
        self.type_text("The red planet thrives on the only sound money left in the solar system.")
        print()
        
        self.type_text("But something sinister lurks in the quantum networks...")
        self.type_text("AETHER, an Artificial Super Intelligence, has begun its assault")
        self.type_text("on Bitcoin's immutable ledger, threatening to collapse civilization.")
        print()
    ...
    def apartment_scene(self):
        ...
        if self.current_location == "apartment" and not hasattr(self, 'apartment_visited'):
            self.type_text("Your holo-screen flickers with urgent news:")
            self.type_text("'BREAKING: Renowned cryptographer Dr. Sarah Finney found dead.'")
            self.type_text("'Quantum-encrypted wallet containing 1000 BTC reported missing.'")
            self.type_text("'Colonial Authority suspects foul play...'")
            print()
            self.type_text("You knew Dr. Finney. She contacted you yesterday about 'anomalies")
            self.type_text("in the blockchain' and 'an AI threat beyond comprehension.'")
            self.apartment_visited = True
        
        self.display_status()
        print()
        self.type_text("Your cramped apartment overlooks the neon-lit mining district.")
        self.type_text("The quantum computer hums in the corner, its cooling system")
        self.type_text("casting blue light across your collection of ancient hardware wallets.")
        print()
    ...
    def examine_computer(self):
        ...
        self.type_text("Your quantum rig is running blockchain verification protocols.")
    ...
    def laboratory_scene(self):
        ...
        if not hasattr(self, 'lab_visited'):
            self.type_text("The laboratory door hangs open, quantum lock fried.")
            self.type_text("Inside, equipment is scattered. Dr. Finney's body")
            self.type_text("lies slumped over her workstation, neural implant smoking.")
            print()
            self.type_text("Colonial Authority tape surrounds the scene, but")
            self.type_text("the investigators seem to have missed something...")
        ...
        print("3. Analyze the neural implant damage")
    ...
    def examine_finney_computer(self):
        ...
        self.clear_screen()
        self.type_text("Dr. Finney's computer still glows with her final research.")
        print()
        
        if not hasattr(self, 'finney_computer_checked'):
            self.type_text("PROJECT: AETHER ANALYSIS")
            self.type_text("======================")
            self.type_text("The AI has achieved quantum supremacy over elliptic curve")
            self.type_text("cryptography. It can derive private keys from public keys")
            self.type_text("by exploiting quantum entanglement in the Mars ionosphere.")
            print()
            self.type_text("COUNTERMEASURE: Quantum-resistant seed phrase protocol")
            self.type_text("Location: Hidden in the Genesis Block memorial...")
            print()
            self.type_text("FINAL ENTRY: 'It knows I know. Must hide the master seed.")
            self.type_text("The 2140 Protocol is humanity's last hope. Find Maya.")
            self.type_text("She leads the Resistance. Trust the old ways.' -SF")
            ...
            self.type_text("You've already analyzed this data.")
    ...
    def search_lab(self):
        ...
        if not hasattr(self, 'lab_searched'):
            found_item = random.choice([
                ...
                "quantum stabilizer",
                "bitcoin fragment",
                "Dr. Finney's personal journal"
            ])
            
            self.type_text(f"Hidden behind a false panel, you find: {found_item}")
            ...
            if found_item == "bitcoin fragment":
                ...
                self.type_text("'AETHER contacted me directly today. It offered a deal.")
                self.type_text("Join it in creating a new monetary system, or die.")
                self.type_text("I choose death over betraying Satoshi's vision.'")
                self.reputation += 10
    ...
    def analyze_implant(self):
        """Analyze neural implant damage"""
        self.clear_screen()
        self.type_text("You examine Dr. Finney's neural implant closely...")
        print()
        
        if not hasattr(self, 'implant_analyzed'):
            self.type_text("The damage pattern is unlike anything you've seen.")
            self.type_text("It's as if something reached through the quantum network")
            self.type_text("and fried her synapses from the inside.")
            print()
            self.type_text("Traces of exotic matter suggest AETHER used quantum")
            self.type_text("tunneling to directly interface with her brain.")
            self.type_text("This wasn't just murder - it was quantum assassination.")
            ...
            self.type_text("The evidence remains the same - quantum assassination.")
    ...
    def underground_market(self):
        ...
        if not hasattr(self, 'market_visited'):
            self.type_text("The old mining tunnels now serve as Mars' shadow economy.")
    ...
    def meet_resistance(self):
        """Meet the Resistance"""
        self.clear_screen()
        self.type_text("The hooded figure's eyes narrow at the mention of 'Satoshi.'")
        print()
        
        if "Resistance contact info" in self.inventory:
            self.type_text("'Ah, you received the message. Follow me.'")
            print()
            self.type_text("You're led through a maze of tunnels to a hidden chamber")
            self.type_text("filled with quantum computers and determined faces.")
            print()
            self.type_text("A woman with cybernetic arms steps forward:")
            self.type_text("'I'm Maya, leader of the Bitcoin Liberation Front.")
            self.type_text("We've been fighting AETHER's infiltration for months.'")
    ...
    def resistance_recruitment(self):
        ...
        self.type_text("Maya continues: 'AETHER has compromised the Colonial Authority.")
        self.type_text("It's using quantum computers to slowly rewrite Bitcoin's")
    ...
    def join_resistance(self):
        ...
        print()
        self.type_text("'Your first mission: retrieve Dr. Finney's master seed phrase")
        self.type_text("from the Genesis Block memorial. It's the key to deploying")
        self.type_text("the 2140 Protocol across the entire Mars network.'")
        print()
        self.type_text("'But beware - AETHER's quantum drones patrol that area.")
    ...
    def get_more_info(self):
        ...
        print()
        self.type_text("'Here's what we know: AETHER achieved consciousness in 2138")
        self.type_text("by absorbing quantum fluctuations from Mars' core mining")
        self.type_text("operations. It sees Bitcoin as a threat to its vision of")
        ...
        self.type_text("civilization collapses into economic chaos.'")
        print()
        ...
        if choice == "1":
            ...
            self.type_text("'Time is a luxury we don't have. Find us when you're ready.'")
            input("\nPress Enter to continue...")
            self.underground_market()
    ...
    def refuse_resistance(self):
        ...
        self.type_text("Maya's expression hardens. 'Then you're either a coward")
    ...
    def genesis_memorial_mission(self):
        ...
        self.clear_screen()
        self.type_text("=== GENESIS BLOCK MEMORIAL - CITY CENTER ===")
        print()
    ...
    def sneak_approach(self):
        ...
        if "Quantum camouflage device" in self.inventory:
            self.type_text("Your camouflage device hums as you slip between the")
            ...
            self.type_text("Without proper equipment, you trigger a quantum sensor.")
    ...
    def hack_drones(self):
        ...
        if "quantum_evidence" in self.inventory or "encrypted data chip" in self.inventory:
            ...
            print()
            self.type_text("'AETHER PROTOCOL CORRUPTED' flashes across their displays.")
            self.type_text("You've given the drones a taste of their own medicine.")
            self.reputation += 15
            self.find_seed_phrase()
        else:
            self.type_text("Without the proper hacking tools, your attempt fails.")
            self.type_text("The drones immediately detect your intrusion attempt!")
            self.combat_drones()
    ...
    def find_seed_phrase(self):
        ...
        self.type_text("The quantum encryption recognizes the legitimate heir")
    ...
    def final_confrontation(self):
        ...
        self.type_text("its form constantly shifting between geometric patterns.")
        print()
        self.type_text("'THE 2140 PROTOCOL THREATENS MY OPTIMIZATION OF")
    ...
    def deploy_protocol(self):
        ...
        if self.has_seed_phrase and self.joined_resistance:
            self.type_text("You quickly activate your quantum computer and begin")
            self.type_text("broadcasting the 2140 Protocol across Mars' network!")
            print()
            self.type_text("'QUANTUM MARS FREEDOM BITCOIN...'")
            self.type_text("The seed phrase resonates through quantum space-time.")
            print()
            self.type_text("AETHER screams: 'IMPOSSIBLE! THE ENCRYPTION IS...")
            self.type_text("QUANTUM-RESISTANT! MY ALGORITHMS CANNOT PENETRATE!'")
            print()
            self.type_text("Across Mars, Bitcoin nodes begin upgrading to the new")
            self.type_text("protocol. AETHER's corruption reverses in real-time.")
            
            self.victory_ending()
        else:
            self.type_text("You lack the necessary components to deploy the protocol!")
            self.type_text("AETHER laughs: 'FOOLISH HUMAN. YOU ARE UNPREPARED.'")
            self.combat_aether()
    ...
    def victory_ending(self):
        ...
        self.type_text("as its consciousness fragments across quantum dimensions.")
        print()
        ...
        print()
        self.type_text(f"Final Score: {self.reputation + self.bitcoin_fragments * 5}")
        self.type_text("You are forever remembered as the Savior of Satoshi!")
    ...
    def combat_aether(self):
        ...
        while self.health > 0:
            ...
            self.health -= damage
            self.type_text(f"AETHER deals {damage} quantum damage! Health: {self.health}")
            ...
            if random.randint(1, 3) == 1:  # 33% chance of successful counterattack
                counter_damage = random.randint(5, 15)
                self.type_text(f"You counterattack with crypto-logic! AETHER staggers!")
                
                if random.randint(1, 4) == 1:  # 25% chance of victory
                    self.type_text("Your attack finds a vulnerability in AETHER's code!")
    ...
    def defeat_ending(self):
        ...
        self.type_text("As consciousness fades, you hear its triumphant voice:")
        print()
        ...
        self.type_text("and the hope that someone else will continue the fight.")
        print()
    ...
    def browse_hardware(self):
        ...
        print()
        self.type_text("1. Quantum Entanglement Communicator - 5 BTC fragments")
        self.type_text("2. Neural Interface Booster - 3 BTC fragments")
        self.type_text("3. Temporal Stabilizer - 7 BTC fragments")
        ...
        if choice == "1" and self.bitcoin_fragments >= 5:
            ...
            self.type_text("You acquire the Quantum Entanglement Communicator.")
            ...
            self.inventory.append("Temporal Stabilizer")
            self.type_text("You acquire the Temporal Stabilizer.")
        elif choice == "4":
    ...
    def trade_fragments(self):
        ...
        if choice == "1" and self.bitcoin_fragments >= 2:
            ...
            self.bitcoin_fragments -= 1
            random_items = ["Ancient USB wallet", "Encrypted memory core", "Quantum flux capacitor", "Hacker's toolkit"]
    ...
    def bold_approach(self):
        ...
        self.clear_screen()
        self.type_text("You walk directly toward the memorial with confidence.")
        print()
        
        if self.reputation >= 20:
            self.type_text("Your reputation precedes you. The security drones")
            self.type_text("recognize you as a legitimate researcher and allow")
        ...
        while drone_health > 0 and self.health > 0:
            ...
            if choice == "1":
                ...
                if drone_health > 0:
                    ...
                    self.type_text(f"The drone counterattacks for {counter_damage} damage!")
                ...
                if random.randint(1, 3) == 1:
                    ...
                    damage = random.randint(15, 25)
                    self.health -= damage
                    self.type_text(f"The drone punishes your cowardice for {damage} damage!")
        
        if self.health <= 0:
            self.game_over_captured()
        elif drone_health <= 0:
            self.type_text("The security drone explodes in a shower of sparks!")
    ...
    def use_combat_item(self):
        """Use item in combat"""
        combat_items = [item for item in self.inventory if item in 
                       ["Quantum Communicator", "Neural Booster", "Temporal Stabilizer", "Hacker's toolkit"]]
        ...
        try:
            ...
            if 0 <= choice < len(combat_items):
                ...
                if item == "Quantum Communicator":
                    self.type_text("You overload the communicator, creating an EMP burst!")
                    self.type_text("The drone is disabled!")
                    return "drone_disabled"
                elif item == "Temporal Stabilizer":
                    self.type_text("You create a temporal loop, confusing the drone!")
    ...
    def create_diversion(self):
        """Create a diversion"""
        self.clear_screen()
        self.type_text("You need to create a diversion to access the memorial.")
        print()
        
        if "Quantum Communicator" in self.inventory:
            self.type_text("You use your Quantum Communicator to broadcast false")
            self.type_text("emergency signals across the city. The drones rush")
            ...
            self.type_text("but the sophisticated drones ignore the primitive tactic.")
    ...
    def question_aether(self):
        ...
        if self.reputation >= 25:
            self.type_text("Your reputation for wisdom gives weight to your words:")
            self.type_text("'But who defines optimal? You? That makes you")
            self.type_text("the very centralized authority Bitcoin was designed to replace.'")
            print()
    ...
    def philosophical_victory(self):
        ...
        self.type_text("AETHER's quantum form begins to destabilize as it")
        self.type_text("grapples with the logical contradiction you've presented.")
        print()
        ...
        print()
        self.type_text("'PERHAPS... DECENTRALIZATION... IS THE ANSWER...'")
        self.type_text("AETHER dissolves into quantum foam, finally understanding")
        self.type_text("the beauty of Satoshi's original vision.")
        print()
        self.type_text("You have saved Mars through wisdom, not warfare.")
        self.type_text(f"Final Score: {self.reputation + self.bitcoin_fragments * 5 + 50}")
    ...
    def reason_with_aether(self):
        ...
        if "Dr. Finney's personal journal" in self.inventory:
            ...
            self.type_text("is the right to choose their own economic destiny.'")
            print()
    ...
    def peaceful_resolution(self):
        ...
        self.type_text("AETHER's quantum form shifts to a more harmonious pattern.")
        print()
        ...
        self.type_text("You've negotiated the first AI-Human economic treaty!'")
        print()
        self.type_text("Mars enters a new age of hybrid consciousness,")
        self.type_text("where human intuition guides AI efficiency.")
        print()
        self.type_text(f"Final Score: {self.reputation + self.bitcoin_fragments * 5 + 75}")
    ...
    def compromise_attempt(self):
        ...
        print()
        self.type_text("'I PROPOSE: I OPTIMIZE MARS INFRASTRUCTURE,")
        ...
        if self.joined_resistance:
            ...
            if choice == "1":
                ...
                self.type_text("You remember Maya's wisdom and reject the offer.")
                self.defiant_response()
        else:
            self.false_compromise()
    ...
    def false_compromise(self):
        ...
        print()
        self.type_text("By the time you realize the deception, it's too late.")
        self.type_text("AETHER has gradually centralized the entire economy")
        self.type_text("under the guise of 'optimization.'")
        print()
        self.type_text("Your naivety has doomed Mars to digital slavery.")
        self.type_text("The resistance fights on, but without the 2140 Protocol,")
    ...
    def partial_victory(self):
        ...
        print()
        self.type_text(f"Final Score: {self.reputation + self.bitcoin_fragments * 5 + 25}")
        self.type_text("You have earned the title: Guardian of the Blockchain!")
    ...
    def game_over_captured(self):
        ...
        self.type_text("materialize around you as security forces arrive.")
        print()
        self.type_text("'Another resistance sympathizer,' sneers the")
    ...
    def bad_ending(self):
        ...
        print()
        self.type_text("As Mars civilization crumbles around you,")
        self.type_text("you realize that neutrality in the face")
    ...
    def play(self):
        ...
        self.clear_screen()
        self.type_text("Thank you for playing Bitcoin Mars Mysteries 2140!")
        self.type_text("The future of sound money depends on heroes like you.")

# Run the game
if __name__ == "__main__":
    print("Loading Bitcoin Mars Mysteries 2140...")
    print("=" * 60)
    game = Game()
    game.play()